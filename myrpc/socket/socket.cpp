...
bool Socket::set_linger(bool active, int seconds)
{
    ...
    {
        log_error("socket set sock linger error: errno=%d errstr=%s", errno, strerror(errno));
        return false;
    }
    return true;
}
...
bool Socket::set_reuse_addr()
{
    ...
    {
        log_error("socket set sock reuser addr error: errno=%s errstr=%s", errno, strerror(errno));
        return false;
    }
    return true;
}
